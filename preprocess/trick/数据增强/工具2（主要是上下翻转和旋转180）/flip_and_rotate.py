...
## pingshen
# path = '/media/clwclw/data/2019bottle/pingshen/aug/'   # root path of data
# name = 'train'    # annotation's name
...
# jiuye
# path = '/media/clwclw/data/2019bottle/jiuye/'   # root path of data
# img_name = 'images_processed4'
# ann_name = 'annotations_jiuye'    # annotation's name,   with prefix .json
path = '/media/clwclw/data/2019bottle/jiuye_zj/'   # root path of data
img_name = 'images'
ann_name = 'annotations'    # annotation's name,   with prefix .json
...
def vflip(dataset, path, name):
    ...
    for img_info in tqdm(dataset['images']):
        ...
        img = Image.open(os.path.join(path, img_name, img_info['file_name']))  # clw note: 根据实际情况选择
        img = img.transpose(1)
...
def xy_exchange(dataset, path, name):
    ...
    for img_info in tqdm(dataset['images']):
        ...
        img = cv2.imread(os.path.join(path, img_name, img_info['file_name']))
...
# 水平翻转
dataset1 = json.load(open(path + '{}.json'.format(ann_name)))
hflip(dataset1, path, ann_name)  # 注意hflip可能会修改dataset,会影响下面的flip,所以搞3份出来

# 垂直翻转
dataset2 = json.load(open(path + '{}.json'.format(ann_name)))
vflip(dataset2, path, ann_name)

# #旋转180
# dataset3 = json.load(open(path + '{}.json'.format(ann_name)))
# rotate180(dataset3, path, ann_name)

#交换xy轴
dataset4 = json.load(open(path + '{}.json'.format(ann_name)))
xy_exchange(dataset4, path, ann_name)

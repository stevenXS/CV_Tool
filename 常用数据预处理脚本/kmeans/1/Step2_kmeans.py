...
class YOLO_Kmeans:
    ...
    def txt2clusters(self):
        ...
        ###
        ratios = []
        for anchor in result_ratio:
            ratios.append(anchor[1] / anchor[0])  # 高宽比，供mmdetection使用
        ###
        ...
        # pass
        return format_anchors, self.avg_iou(all_boxes, result) * 100, sorted(ratios)  # clw modify
...
def kmeans_anchors(filename, cluster_number):
    ...
    ratios_max = 0
    kmeans = YOLO_Kmeans(cluster_number, filename)
    anchors_max, acc_max, ratios = kmeans.txt2clusters()
    for i in range(0, 100): #clw modify:多次聚类找到最佳结果
        print('clw: iter', i)
        kmeans = YOLO_Kmeans(cluster_number, filename)
        anchors, acc, ratios = kmeans.txt2clusters()
        if acc > acc_max:
            acc_max = acc
            anchors_max = anchors
            ratios_max = ratios
    print("\nK anchors: {}".format(anchors_max))
    print("Accuracy: {:.2f}%".format(acc_max))  # clw note
    print('高宽比 for mmdetection:', ratios_max)

if __name__ == "__main__":
    # 里面是一系列图片文件路径和标注信息,可以用voc2kemansTxt.py生成
    filename = "train.txt"
    cluster_number = 27  # anchor的个数,默认9

...
# 功能说明：
# 输入：包括图片存放路径 source_pic_root_path
#           xml存放路径 source_xml_root_path
#           增强后的图片存放文件夹，默认 augment_save_folder = 'augment'
# 输出： 在augment文件夹内生成增强后的图片
...
def show_pic(img, bboxes=None):
    ...
    输入:
        ...
        names:每个box对应的名称
    '''
    cv2.imwrite('./1.jpg', img) # clw note：搞一个临时文件，防止展示原图的时候img已经带了bbox框，没法再用于后面数据增强然后展示
...
if __name__ == '__main__':
    ...
    '''xml类型的数据'''
    source_pic_root_path = 'D:/textile/test/'

...
category_count_dict = {}
dataset_path = 'D:/pear_4'

ann_file_count = 1
for root, dirs, files in os.walk(dataset_path):
    ann_files = [file for file in files if file.endswith('.json')]
    for ann_file in ann_files:
        ann_file_path = os.path.join(root, ann_file)
        print('index %d, ann_file_path: %s' % (ann_file_count, ann_file_path))
        ann_file_count += 1
        ### 提取 obj 里面的 label信息
        with open( ann_file_path ) as f:
            ann = json.load(f)
            for j, obj in enumerate(ann['shapes']):
                label = obj['label']
                if label in category_count_dict:
                    category_count_dict[label] += 1;
                else:
                    category_count_dict[label]= 0;

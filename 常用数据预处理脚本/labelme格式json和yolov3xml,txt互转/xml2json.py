...
def xml2json(xml_file_path, json_path, img_path):
    ...
    for obj in objs:
        ...
        x1 = float(bndbox_obj.find('xmin').text)
        y1 = float(bndbox_obj.find('ymin').text)
        x2 = float(bndbox_obj.find('xmax').text)
        y2 = float(bndbox_obj.find('ymax').text)
        ...
        for f in fs:
            ...
            if fmt == '.xml':
                ...
                if img_path is not None:
                    xml2json(xmlf_path, json_path, img_path)
                else:
